...
"""Convert authoritative MCore routes into compact megakernel inputs."""
...
def _routing_map_to_indices_backward_kernel(
    ...
    tl.store(
        grad_probs_ptr + grad_probs_row_offset + expert_offsets, 0.0, mask=expert_mask
    )
    tl.debug_barrier()
    topk_offsets = tl.arange(0, topk_block_size)
    topk_mask = topk_offsets < topk
...
class RoutingMapToIndices(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, probs, routing_map, topk, index_dtype):
        if not HAVE_TRITON:
            raise RuntimeError("routing_map_to_compact_inputs requires Triton")
        ...
        if (
            not probs.is_cuda
            or not routing_map.is_cuda
            or probs.device != routing_map.device
        ):
            raise ValueError(
                "probs and routing_map must be CUDA tensors on the same device"
            )
        ...
        if type(topk) is not int or topk <= 0 or topk > probs.shape[1]:
            raise ValueError(
                "topk must be a positive integer no greater than num_experts"
            )
        ...
        indices = torch.empty(
            (num_tokens, topk), dtype=index_dtype, device=probs.device
        )
        if num_tokens > 0:
...
def routing_map_to_compact_inputs(
    probs: torch.Tensor,
    ...
    *,
    ...
    """Compact an authoritative routing map into fixed-width backend inputs.
    Args:
        ...
        index_dtype: Integer dtype required by the selected backend.
    Returns:
        A pair (probs_indices, indices) with shapes [num_tokens, topk].
        probs_indices is float32 and indices uses ``index_dtype`` with -1 padding.
    """
    return RoutingMapToIndices.apply(probs, routing_map, topk, index_dtype)
