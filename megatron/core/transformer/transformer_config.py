...
class TransformerConfig(ModelParallelConfig):
    ...
    experimental_attention_variant_loss_scale_func: Optional[
    ...
    moe_flex_dispatcher_backend: Literal["deepep", "deepepv2", "hybridep", "ncclep"] = (
        "deepep"
    )
    ...
    thd_max_packed_sequences: Optional[int] = field(
        default=None,
        metadata={"argparse_meta": {"arg_names": ["--thd-max-packed-sequences"]}},
    )
    ...
    def __post_init__(self):
        ...
        if (
            self.gated_attention_proj_granularity == "headwise"
        ...
        if is_gated_delta_net_variant(self.experimental_attention_variant):
            ...
            if self.experimental_attention_variant == "kda":
                ...
                if self.linear_key_head_dim != self.linear_value_head_dim:
                    raise ValueError(
                        "KDA requires equal key and value head dimensions."
                    )
                if self.kda_safe_gate:
                    if self.kda_lower_bound is None:
                        raise ValueError(
                            "KDA requires kda_lower_bound when kda_safe_gate=True."
                        )
        ...
        if self.experimental_attention_variant == "dsa":
            ...
            assert self.multi_latent_attention, (
                "DSv4 Hybrid requires multi_latent_attention."
            )
            assert self.csa_compress_ratios is not None, (
                "csa_compress_ratios must be set"
            )
            ...
            if self.dsa_kernel_backend == "cudnn":
                ...
                if (
                    ...
                    missing_offsets = [
                        ...
                        if "q_causal_offsets"
                        not in inspect.signature(wrapper).parameters
                    ]
                    if missing_offsets:
                        raise ValueError(
        ...
        if (
            ...
            and self.experimental_attention_variant == "kda"
        ):
        ...
        if self.moe_single_grouped_weight:
            ...
            if (
                ...
                and self.moe_megakernel_backend
                not in (
                    "mok",
                    "fk",
                )
        ...
        if self.moe_token_dispatcher_type == "flex":
            if self.moe_pad_expert_input_to_capacity and (
                ...
                or self.moe_flex_dispatcher_backend in ("deepep", "deepepv2")
            ):
                raise ValueError(
        ...
        if _deprecated_num_sms:
            ...
            if self.moe_flex_dispatcher_num_sms is None:
                ...
                self.moe_flex_dispatcher_num_sms = next(
                    iter(_deprecated_num_sms.values())
                )
        ...
        if self.moe_megakernel_backend == "mok":
            ...
            if self.moe_shared_expert_gate or self.moe_shared_expert_overlap:
                raise ValueError(
                    "MOK does not support the MCore shared-expert gate/overlap"
                )
        ...
        if self.moe_megakernel_backend == "fk":
            if not self.gradient_accumulation_fusion:
                raise ValueError(
                    "FK currently requires gradient_accumulation_fusion=True"
                )
            fk_mxfp8 = (
                self.fp8 is not None
                and self.fp8_recipe == Fp8Recipe.mxfp8
            ...
            if self.moe_shared_expert_gate or self.moe_shared_expert_overlap:
                raise ValueError(
                    "FK does not support the MCore shared-expert gate/overlap yet"
                )
            if self.moe_latent_size is not None:
            ...
            if (
                self.fk_bwd_workspace_reset_mode
                not in supported_fk_workspace_reset_modes
            ):
                raise ValueError(
            ...
            if len(self.fk_bwd_epi_flag_batch) != 2 or any(
                ...
                raise ValueError(
                    "fk_bwd_epi_flag_batch must contain two positive integers"
                )
        ...
        if (
            ...
            if self.fine_grained_activation_offloading and self.offload_modules:
                ...
                conflicting = _MHC_CONFLICTING_OFFLOAD_MODULES & set(
        ...
        if self.fine_grained_activation_offloading:
            ...
            assert (
                self.activation_offload_fraction >= 0
                and self.activation_offload_fraction <= 1
            ...
            if self.fine_grained_offloading_max_inflight_offloads is not None:
                assert self.fine_grained_offloading_max_inflight_offloads >= 0, (
            ...
            if "fused_group_mlp" in self.offload_modules:
                ...
                moe_partial_offload = {"expert_fc1", "moe_act"} & set(
                    self.offload_modules
                )
            ...
            if self.fine_grained_activation_offloading:
                ...
                if self.cuda_graph_impl == "local":
                    ...
                    unsupported_offload_modules = (
                        offload_modules - local_supported_offload_modules
                    )
        ...
        if self.num_moe_experts is not None and self.moe_token_dispatcher_type in [
            "allgather"
        ]:
        ...
        if self.overlap_moe_expert_parallel_comm:
            ...
            if self.recompute_granularity == "full":
                ...
                assert self.recompute_method is None, (
                    "disable recomputation method when enabling "
        ...
        if self.sequence_packing_scheduler is not None:
            ...
            if not (
                is_te_min_version("2.9.0")
                or get_te_version() == PkgVersion("2.9.0.dev0+5b3092a")
            ):
                raise ValueError(
...
class MLATransformerConfig(TransformerConfig):
    ...
    def __post_init__(self):
        ...
        if self.experimental_attention_variant == "dsv4_hybrid":
            assert not self.mla_down_proj_fusion, (
