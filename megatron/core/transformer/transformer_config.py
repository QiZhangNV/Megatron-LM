...
class TransformerConfig(ModelParallelConfig):
    ...
    "gdn_norm_out": recompute the GatedDeltaNet output norm and HP-to-CP all-to-all as a
            discard-output checkpoint. Requires experimental_attention_variant="gated_delta_net"
            and cannot be combined with "gdn".
    "gdn_qkv": recompute the GatedDeltaNet QKV projection and preparation block as a
            discard-output checkpoint. Requires experimental_attention_variant="gated_delta_net"
    ...
    def __post_init__(self):
        ...
        if self.recompute_granularity == "selective":
            ...
            if "gdn" in self.recompute_modules and "gdn_qkv" in self.recompute_modules:
                raise ValueError(
                    "gdn and gdn_qkv in recompute_modules cannot be used together: "
                    "'gdn' already recomputes the entire GatedDeltaNet module."
                )
